 ...
 */
void InitDisplay(int argc, char **argv) { InitWindow(argc, argv); }
 ...
 */
void StartDisplay() { StartWindow(ratBits_width, ratBits_height); }
...
void ShowView(Loc x, Loc y, Direction dir) {
  ...
  if (prevEdge3) (void)plotLine(edge3Lines, TRUE);
  if (prevEdge7) (void)plotLine(edge7Lines, TRUE);

  /* show the tokens */

  for (ratIndex = 0; ratIndex < MAX_RATS;
       ratIndex = RatIndexType(ratIndex.value() + 1)) {
    if (ratIndex == MY_RAT_INDEX) continue;
    ratLook = &Rats2Display[ratIndex.value()];
    oldVisible = ratLook->visible;
    TokenVisible(ratIndex);
    if (ratLook->visible == TRUE) XORToken(ratIndex);
    if (ratLook->visible != oldVisible) UpdateScoreCard(ratIndex);
  }
}

/* ----------------------------------------------------------------------- */

static XYpair *plotLine(XYpair *p, bool two) {
  while (1) {
...
static XYpair *hidden(Loc x, Loc y, Direction dir, XYpair *p) {
  int l1x, l1y, l2x, l2y;
  int r1x, r1y, r2x, r2y;
  int c2x, c2y;
...
static void getRat(RatIndexType hisRatIndex, int *x, int *y, int *size) {
  ...
  switch (Rats2Display[hisRatIndex.value()].distance) {
    case 1:
      ...
      *x = 0;
      *y = 0;
      for (viewT = 0; viewT != view; *x += 64) viewT++;
      return;
...
void clearPosition(RatIndexType ratIndex, Loc xClear, Loc yClear) {
  ...
  for (i = 0; i < MAX_RATS; i++) {
    rp = M->rat(i);
    if ((i == ratIndex.value()) || !(rp.playing)) continue;
    if ((rp.x == xClear) && (rp.y == yClear))
      if (i == MY_RAT_INDEX) showMe(rp.x, rp.y, rp.dir);
  }
}
...
void UpdateScoreCard(RatIndexType ratIndex) {
  ...
  if (M->rat(ratIndex).playing) WriteScoreString(ratIndex);
  if (Rats2Display[ratIndex.value()].visible == TRUE) InvertScoreLine(ratIndex);
}
...
void byteSwap(register BitCell *bits, int size) {
  ...
  for (i = 0; i < size; i++, bits++)
    for (j = 0; j < 16; j++) bits->bits[j] = htons(bits->bits[j] & 0xffff);
}

/* ----------------------------------------------------------------------- */

/*
 * Displays a missile.  Takes the new location to display the missile in x_loc
 * and y_loc
 * Takes the previous location of the missile to delete, if the clear boolean is
 * set
 */
