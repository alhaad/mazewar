...
static XtResource resources[] = {
    ...
    {XtNmouse, XtCCursor, XtRPixel, sizeof(Pixel), offset(mouseground),
     XtRString, strdup("black")},
    {XtNfont, XtCFont, XtRFont, sizeof(Font), offset(scoreFont), XtRString,
     strdup(DEFAULT_FONT)},
    {XtNborderWidth, XtCBorderWidth, XtRInt, sizeof(int), offset(borderWidth),
     XtRInt, (caddr_t)(void *)&one},
    {XtNborder, XtCBorderColor, XtRPixel, sizeof(Pixel), offset(borderColor),
     XtRString, strdup("black")},
};
#undef offset
...
void InitWindow(int argc, char **argv) {
  ...
  w = XtAppInitialize(&app_con, progName, opTable, XtNumber(opTable), &argc,
                      argv, default_resources, NULL, ZERO);
  ...
  if (!app_resources.scoreFont) MWError("cannot open font");
  ...
  cur_height = MIN_Y_DIM +
               (MAX_RATS + 1) * (scoreFontInfo->max_bounds.ascent +
  ...
  copyGC = XCreateGC(dpy, mwWindow, GCFunction | GCForeground | GCBackground |
                                        GCLineWidth | GCFont,
                     &gc_values);
  ...
  icon_pixmap = XCreatePixmapFromBitmapData(
      dpy, mwWindow, (char *)icon_bits, icon_width, icon_height,
  ...
  iconmask_pixmap = XCreatePixmap(dpy, mwWindow, icon_width, icon_height,
                                  XDefaultDepth(dpy, screen));
  XFillRectangle(dpy, iconmask_pixmap, iconGC, 0, 0, icon_width, icon_height);

  icon_reverse_pixmap = XCreatePixmapFromBitmapData(
      dpy, mwWindow, (char *)icon_bits, icon_width, icon_height,
      app_resources.bg_pixel, app_resources.fg_pixel,
      XDefaultDepth(dpy, screen));
 ...
 */
void ClearView(void) {
  XClearArea(dpy, mwWindow, VIEW_X_ORIGIN, VIEW_Y_ORIGIN, VIEW_X_DIM,
             VIEW_Y_DIM, False);
}
...
static short RandomEvent(void) {
  ...
  if ((choice == 0) || M->peeking()) {
    for (ratIndex = RatIndexType(0); ratIndex.value() < MAX_RATS;
  ...
  while (1) {
    ...
    if (!XPending(dpy)) {
      ...
      for (ret = 0; ret <= 0;) {
        ...
        if (ret == -1) {
          if (errno != EINTR) MWError("select error on events");
        } else if (ret == 0) {
          nexttimeout.tv_sec = app_resources.time_interval / 1000;
          nexttimeout.tv_usec = 1000 * (app_resources.time_interval % 1000);
          if (app_resources.robotic == TRUE)
...
#define LeftButton Button1
    ...
    if (FD_ISSET(socket, &fdmask)) {
      ...
      if (cc <= 0) {
        if (cc < 0 && errno != EINTR) perror("event recvfrom");
        continue;
      }
      if (fromLen != sizeof(struct sockaddr_in)) continue;
...
bool KBEventPending(void) { return (XPending(dpy) != 0); }
...
void DisplayRatBitmap(int screenX, int screenY, int width, int height, int srcX,
  ...
  XPutImage(dpy, mwWindow, xorGC, ratsImage, srcX, srcY,
            screenX + VIEW_X_ORIGIN, screenY + VIEW_Y_ORIGIN, width, height);
#endif /* USE_BITMAPS */
}
  ...
  XClearArea(dpy, mwWindow, SCORE_X_ORIGIN,
             SCORE_Y_ORIGIN +
                 rat.value() * (scoreFontInfo->max_bounds.ascent +
                                scoreFontInfo->max_bounds.descent),
             SCORE_X_DIM, (scoreFontInfo->max_bounds.ascent +
                           scoreFontInfo->max_bounds.descent),
             FALSE);
...
void InvertScoreLine(RatIndexType rat) {
  XFillRectangle(dpy, mwWindow, xorGC, SCORE_X_ORIGIN,
                 SCORE_Y_ORIGIN +
                     rat.value() * (scoreFontInfo->max_bounds.ascent +
                                    scoreFontInfo->max_bounds.descent),
                 SCORE_X_DIM, scoreFontInfo->max_bounds.ascent +
 ...
 */
void NotifyPlayer(void) { flashIcon = TRUE; }
...
static void initCursors(void) {
  ...
  m = XCreateBitmapFromData(dpy, mwWindow, (char *)ratMask_bits, ratMask_width,
                            ratMask_height);
  p = XCreateBitmapFromData(dpy, mwWindow, (char *)rat_bits, rat_width,
                            rat_height);
  ratCursor = XCreatePixmapCursor(dpy, p, m, &mc, &bc, rat_x_hot, rat_y_hot);

  m = XCreateBitmapFromData(dpy, mwWindow, (char *)dRatMask_bits,
                            dRatMask_width, dRatMask_height);
  p = XCreateBitmapFromData(dpy, mwWindow, (char *)dRat_bits, dRat_width,
                            dRat_height);
  deadRatCursor = XCreatePixmapCursor(dpy, p, m, &mc, &bc, 0, 0);

  p = XCreateBitmapFromData(dpy, mwWindow, (char *)cup_bits, cup_width,
                            cup_height);
...
#else
  mazeImage =
      XCreateImage(dpy, DefaultVisual(dpy, DefaultScreen(dpy)), 1, XYBitmap, 0,
                   mazeBits, MAZE_X_DIM, MAZE_Y_DIM, 8, MAZE_X_DIM >> 3);
...
void drawMaze() {
#ifdef USE_BITMAPS
  XCopyPlane(dpy, mazeBitmap, mwWindow, copyGC, 0, 0, MAZE_X_DIM, MAZE_Y_DIM,
             MAZE_X_ORIGIN, MAZE_Y_ORIGIN, 1);
#else
  XPutImage(dpy, mwWindow, copyGC, mazeImage, 0, 0, MAZE_X_ORIGIN,
...
#ifdef USE_BITMAPS
  ratsBitmap =
      xCreateBitmapFromBitCell(dpy, mwWindow, (char *)ratBits, width, height);
  if (ratsBitmap == 0)
#else
  ratsImage =
      XCreateImage(dpy, DefaultVisual(dpy, DefaultScreen(dpy)), 1, XYBitmap, 0,
                   (char *)ratBits, width, height, 8, width >> 3);
